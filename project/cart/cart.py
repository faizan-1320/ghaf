...
def add_to_cart(product_id):
    ...
    cursor = g.db.cursor()
    cursor.execute(
        "select product_id from tbl_cart where product_id = %s and user_id=%s", (product_id, user_id))
    is_product_in_cart = cursor.fetchone()
    print(not is_product_in_cart)
    if not is_product_in_cart:
        cursor.execute(
            'INSERT INTO tbl_cart(user_id,product_id,quentity) VALUES(%s,%s,%s)', (user_id, product_id, 1))
        g.db.commit()
...
def remove_form_cart(product_id):
    ...
    cursor = g.db.cursor()
    cursor.execute(
        "DELETE FROM tbl_cart WHERE product_id = %s and user_id=%s", (product_id, user_id))
    g.db.commit()
...
def increase_quentity(product_id):
    ...
    cursor = g.db.cursor()
    cursor.execute(
        "SELECT id FROM tbl_category_product WHERE stock >= quentity and id = %s", (product_id,))
    stock = cursor.fetchone()
    if stock:
        cursor.execute(
            "SELECT p.stock, p.quentity * c.quentity ,p.quentity FROM tbl_category_product AS p JOIN tbl_cart AS c on p.id = c.product_id WHERE p.id = %s", (product_id,))
        ...
        if check_stock[0] > check_stock[1]:
            cursor.execute(
                "UPDATE tbl_cart SET quentity = quentity+1 WHERE product_id = %s and user_id=%s", (product_id, user_id))
            g.db.commit()
...
def decrease_quentity(product_id):
    ...
    cursor = g.db.cursor()
    cursor.execute(
        "select quentity from tbl_cart where product_id = %s and user_id=%s", (product_id, user_id))
...
def display_cart_product():
    ...
    cursor.execute(
        'SELECT service_id FROM tbl_falicity_management WHERE user_id=%s', (user_id,))
    ...
    if falicity_id:
        if len(falicity_id) == 1:
            cursor.execute(
                'SELECT facility_charges FROM tbl_additional_facility WHERE id = %s', (falicity_id))
    ...
    cursor.execute(
        "select firstname,lastname from tbl_users where id = %s", (user_id,))
    name = cursor.fetchone()
    print(count)
    products = [{"product_id": data[0], "quentity":data[1], "price":data[2], "product_unit":data[3],
                 "quentity":data[4], "product_name":data[5], "subtotel":data[6], "image":data[7]} for data in cart_data]
    sub_total = 0
    print(products)
    for i in products:
...
def add_facility_or_remove(product_id, service_id):
    user_id = get_jwt_identity()
    cursor = g.db.cursor(buffered=True)
    cursor.execute('SELECT * FROM tbl_falicity_management WHERE service_id=%s and product_id=%s and user_id=%s',
                   (service_id, product_id, user_id))
    already_apply = cursor.fetchone()
    if already_apply:
        cursor.execute('DELETE FROM tbl_falicity_management WHERE service_id=%s and product_id=%s and user_id=%s',
                       (service_id, product_id, user_id))
        g.db.commit()
        return jsonify({"massage": "service delete"})
    cursor.execute('INSERT INTO tbl_falicity_management (service_id,product_id,user_id) VALUES(%s,%s,%s)',
                   (service_id, product_id, user_id))
    g.db.commit()
