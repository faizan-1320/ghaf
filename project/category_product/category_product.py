...
def category_product_header(service_category_name):
    try:
        ...
        cursor.execute(
            f"SELECT sc.id, sc.category_name, s.service from tbl_service_category sc JOIN tbl_service s ON sc.service_id = s.id WHERE category_name='{service_category_name}'")
        ...
        cursor.execute(
            f"SELECT COUNT(id) as Items from tbl_category_product WHERE service_category_id=4 AND is_active = 1 AND is_delete = 0;")
...
def category_product_search(service_category_name):
    try:
        ...
        if service == 'fish-market':
            print(search)
            cursor.execute(
                f"SELECT p.id,p.product_name, p.price, p.quantity, p.product_unit,(SELECT pi.image FROM tbl_product_image pi WHERE pi.product_id = p.id LIMIT 1) AS Product_image from tbl_category_product p WHERE p.service_category_id={service_category_id} AND p.product_name LIKE '%{search}%' AND p.is_active = 1 AND p.is_delete = 0;")
            data = cursor.fetchall()
            cursor.execute(
                f"SELECT id, facility_name, facility_charges FROM tbl_additional_facility WHERE id=1 AND is_active=1 AND is_delete=0 ")
...
def category_product(service_category_name):
    try:
        ...
        if service == 'fish-market':
            cursor.execute(
                f"SELECT p.id,p.product_name, p.price, p.quantity, p.product_unit,(SELECT pi.image FROM tbl_product_image pi WHERE pi.product_id = p.id LIMIT 1) AS Product_image from tbl_category_product p WHERE p.service_category_id={service_category_id} AND p.is_active = 1 AND p.is_delete = 0;")
            data = cursor.fetchall()
            cursor.execute(
                f"SELECT id, facility_name, facility_charges FROM tbl_additional_facility WHERE id=1 AND is_active=1 AND is_delete=0 ")
            additional_data = cursor.fetchall()
...
def category_product_details(product_id):
    try:
        ...
        cursor.execute(
            f"SELECT s.service, service_category_id FROM tbl_category_product p JOIN tbl_service_category sc ON p.service_category_id = sc.id JOIN tbl_service s ON sc.service_id = s.id WHERE p.id={product_id};")
        data = cursor.fetchone()
        service = data['service']
        service_category_id = data['service_category_id']
        cursor.execute(
            f"SELECT image FROM tbl_product_image WHERE product_id={product_id} AND is_active=1 AND is_delete=0 ")
        product_image = cursor.fetchall()
        if service == 'fish-market':
            cursor.execute(
                f"SELECT p.product_name, s.name as storename, p.price, p.product_unit, p.quantity FROM tbl_category_product p JOIN tbl_store s ON p.store_id = s.id WHERE p.id={product_id} AND p.is_active=1 AND p.is_delete=0;")
            product_details = cursor.fetchall()
            cursor.execute(
                f"SELECT facility_name, facility_charges FROM tbl_additional_facility WHERE id != 0")
            ...
            cursor.execute(
                f"SELECT p.product_name, p.price, p.product_unit, p.quantity, p.description FROM tbl_category_product p WHERE p.id={product_id} AND p.is_active=1 AND p.is_delete=0;")
            product_details = cursor.fetchall()
