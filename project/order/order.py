...
def add_new_address_villa():
    ...
    cursor.execute(
        'SELECT *  FROM tbl_users WHERE id = %s AND is_active=1 and is_delete=0 AND is_verify=1', (1,))
        ...
        if user_data["user_type"] == "user":
            if user_data["is_verify"] == 1:
                ...
                if save_addresh == "yes" or "YES" or "y" or "Y" or "Yes":
                    cursor.execute("INSERT INTO tbl_user_address (user_id,address_type,property_type,property_number,landmark,area,street,city,latitude,longitude) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", (
                        1, address_type, property_type, property_number, landmark, area, street, city, latitude, longitude))
                    g.db.commit()
    ...
    if user_data:
        if user_data["user_type"] == "user":
            if user_data["is_verify"] == 1:
                ...
                apartment_floor_number = request.json.get(
                ...
                cursor = g.db.cursor(dictionary=True)
                if save_addresh == "yes" or "YES" or "y" or "Y" or "Yes":
                    cursor.execute("INSERT INTO tbl_user_address (user_id,address_type,property_type,property_number,landmark,area,street,city,latitude,longitude,name,apartment_floor_number) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", (
                        1, address_type, property_type, property_number, landmark, area, street, city, latitude, longitude, name, apartment_floor_number))
                    g.db.commit()
                    return jsonify({"message": "Added Address"})
            else:
                return jsonify({"message": "You are not verify"})
    ...
    if user_data:
        if user_data["user_type"] == "user":
            if user_data["is_verify"] == 1:
                ...
                cursor = g.db.cursor(dictionary=True)
                if save_addresh == "yes" or "YES" or "y" or "Y" or "Yes":
                    cursor.execute("INSERT INTO tbl_user_address (user_id,address_type,property_type,property_number,area,street,city,latitude,longitude,name,section,apartment_floor_number) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", (
                        1, address_type, property_type, property_number, area, street, city, latitude, longitude, name, section, apartment_floor_number))
                    g.db.commit()
                    return jsonify({"message": "Added Address"})
            else:
                return jsonify({"message": "You are not verify"})
        ...
        if user_data["user_type"] == "user":
            if user_data["is_verify"] == 1:
                ...
                if falicity_id:
                    ...
                    cursor.execute(
                        f'SELECT facility_charges FROM tbl_additional_facility WHERE id in {falicity_id}')
                ...
                cursor.execute(
                    "select count(id) from tbl_cart where user_id = %s", (user_id,))
                ...
                cursor.execute(
                    f"SELECT id FROM `tbl_user_address` WHERE user_id= %s GROUP BY id desc LIMIT 1;", (1,))
                address_data = cursor.fetchone()
                print("---------------------------------->", address_data)
                convert_datetime_str_to_datetime = datetime.strptime(
                    delivery_date, '%d-%m-%Y')
                convert_datetime_to_date = convert_datetime_str_to_datetime.date()
                now = datetime.now()
                order_date = now.date()
                cursor.execute(
                    f"SELECT * FROM tbl_promocode WHERE id = %s;", (promocode,))
                ...
                if promocode_discount["discount_type"] == "percentage":
                    if order_date < convert_datetime_to_date:
                        ...
                        promocode_discount_value = sub_total * \
                            promocode_discount["discount_value"]/100
                        ...
                        # update stock detail
                        cursor.execute(
                            'select * from tbl_order_details where order_number=%s;', (order_number_,))
                        oreder_detail = cursor.fetchall()
                        print("oreder_detail", oreder_detail)
                        for i in oreder_detail:
                            cursor.execute(
                                'SELECT id,stock FROM tbl_category_product WHERE id=%s', (i["product_id"],))
                            stock_data = cursor.fetchone()
                            updated_stock = stock_data["stock"] - i["quentity"]
                            # print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>.",updated_stock)
                            print(stock_data["id"], stock_data["stock"])
                            cursor.execute(
                                'UPDATE tbl_category_product SET stock = %s WHERE id = %s;', (updated_stock, i["product_id"]))
                            g.db.commit()
                        ...
                        cursor.execute(
                            'select id from tbl_order WHERE order_number=%s;', (order_number_,))
                        order_id = cursor.fetchone()

                        if receiver_type == "i-am":
                            print(order_id)
                            cursor.execute('INSERT INTO tbl_order_receiver (order_id,order_number,reciver_id) VALUES (%s,%s,%s)', (
                                order_id['id'], order_number_, address_data['id']))
                            ...
                            for i, j in zip(receiver_detail["reciver_name"], receiver_detail["reciver_number"]):
                                cursor.execute(
                                    'INSERT INTO tbl_receiver (user_id,name,phone_number) VALUES (%s,%s,%s)', (user_id, i, j))
                                g.db.commit()
                                cursor.execute(
                                    'select id from tbl_receiver order by id desc limit 1')
                                recever_id = cursor.fetchone()
                                # print(recever_id)
                                # return "hihihi"
                                cursor.execute('INSERT INTO tbl_order_receiver (order_id,order_number,reciver_id) VALUES (%s,%s,%s)', (
                                    order_id['id'], order_number_, recever_id['id']))
                                g.db.commit()
                            return jsonify({"massage": 'data update sucessfully'})
